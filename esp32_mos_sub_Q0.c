                               ...
                               int32_t event_id, void *event_data)
{
    ...
    if (event->event_id == MQTT_EVENT_CONNECTED) {
        ...
        esp_mqtt_client_subscribe(event->client, "iot/test1", 0);
    ...
    if (event->event_id == MQTT_EVENT_DATA) {
        ...
        if (!(bits & TIME_SYNCED_BIT)) {
            ESP_LOGW(TAG, "Data received but time not synced yet");
            return;
        }
        ...
        unsigned long pkt;
        unsigned long long sent_ts;

        if (sscanf(payload, "%lu,%llu", &pkt, &sent_ts) == 2) {
            uint64_t now = get_time_ms();
            ESP_LOGI(TAG, ", %lu, %llu", pkt, now - sent_ts);
        }
    }
}
...
void app_main(void)
{
    ...
    esp_mqtt_client_config_t mqtt_cfg = {
        //.broker.address.uri = "mqtt://192.168.88.122:1883",
        .broker.address.uri = "mqtt://10.52.5.136:1883",
    };
